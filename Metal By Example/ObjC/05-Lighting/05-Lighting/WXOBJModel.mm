...
// face vertices 是元祖 索引, 文件宽度  (position normals)
//
//
//
struct FaceVertex
...
{
    __block WXOBJGroup *group = nil;
    [_mutableGroups enumerateObjectsUsingBlock:^(WXOBJGroup *obj, NSUInteger idx, BOOL *stop)
     {
...
- (void)beginGroupWithName:(NSString *)name
{
    ...
    WXOBJGroup *newGroup = [[WXOBJGroup alloc] initWithName:name];
...
- (void)endCurrentGroup
{
    ...
    NSData *vertexData = [NSData dataWithBytes:groupVertices.data() length:sizeof(WXVertex) * groupVertices.size()];
    self.currentGroup.vertexData = vertexData;
    
    NSData *indexData = [NSData dataWithBytes:groupIndices.data() length:sizeof(WXIndex) * groupIndices.size()];
...
- (void)generateNormalsForCurrentGroup
{
    ...
    for (i = 0; i < indexCount; i += 3)
    {
        ...
        WXVertex *v0 = &groupVertices[i0];
        WXVertex *v1 = &groupVertices[i1];
        WXVertex *v2 = &groupVertices[i2];
